...
function checkPackageJson() {
  ...
  try {
    ...
    addResult(
      ...
      hasPreDeploy,
      hasPreDeploy
        ? "✓ pre-deploy script exists"
        : "⚠ pre-deploy script missing (recommended)",
      "warning",
    );
  } catch (error) {
...
function checkLicense() {
  ...
  if (licenseExists) {
    ...
    addResult(
      ...
      hasContent
        ? "✓ License has content"
        : "✗ License appears to be placeholder",
      "warning",
    );
  }
}
  ...
  if (readmeExists) {
    ...
    for (const section of sections) {
      ...
      addResult(
        ...
        hasSection,
        hasSection
          ? `✓ ${section.name} section found`
          : `⚠ ${section.name} section missing`,
        "warning",
      );
    }
  }
...
function checkRoadmap() {
  ...
  if (roadmapExists) {
    ...
    addResult(
      ...
      hasPhases,
      hasPhases
        ? "✓ Roadmap has phases/milestones"
        : "⚠ Roadmap missing structure",
      "info",
    );
  }
}
...
function checkEnvExample() {
  const envExampleExists = checkFileExists(
    ".env.example",
    "Environment example",
    false,
  );
  ...
  if (envExampleExists) {
    ...
    const hasRealSecrets = /sk-[a-zA-Z0-9]{20,}|postgres:\/\/.*@.*\.com/.test(
      content,
    );
    addResult(
...
function checkTypeScript() {
  const tsconfigExists = checkFileExists(
    "tsconfig.json",
    "TypeScript config",
    false,
  );
...
function checkReleaseNotes() {
  const releaseNotesExists = checkFileExists(
    "RELEASE_NOTES.md",
    "Release notes",
    false,
  );
  ...
  if (releaseNotesExists) {
    ...
    addResult(
      ...
      hasVersion,
      hasVersion
        ? "✓ Release notes have version info"
        : "⚠ No version info in release notes",
      "info",
    );
  }
}

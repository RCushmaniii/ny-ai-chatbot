 ...
 */
export async function getDailyUsageStats(date: Date): Promise<DailyUsageStats | null> {
  try {
...
export function formatUsageReportEmail(stats: DailyUsageStats): string {
  const totalLanguageMessages = stats.languageBreakdown.en + stats.languageBreakdown.es;
  const enPercent = totalLanguageMessages > 0 
    ? Math.round((stats.languageBreakdown.en / totalLanguageMessages) * 100) 
    : 0;
  const esPercent = totalLanguageMessages > 0 
 ...
 */
export async function sendUsageReportEmail(stats: DailyUsageStats, toEmail: string): Promise<void> {
  try {
